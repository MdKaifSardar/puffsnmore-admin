...
const ProductUpdate = ({ productId }: { productId: string }) => {
  ...
  const handleProductUpdateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await updateProduct(
        productId, // Ensure you have productId available in the component
        sizes,
        discount,
        name,
        description,
      ...
      if (result?.success) {
        ...
        toast.error(result?.message || "Failed to update the product.", {
          position: "top-right",
          autoClose: 5000,
          hideProgressBar: false,
          closeOnClick: true,
