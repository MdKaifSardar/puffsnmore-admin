...
// get single sub category for amdin
...
export const deleteSubCategory = async (id: string) => {
  try {
    ...
    if (!subCategory) {
      console.log("Sub Category not found with this ID!");
      return {
        success: false,
    ...
    const imagePublicIds = subCategory.images
      .filter((image: any) => image?.public_id) // Ensure only valid public_ids are used
    ...
    // Fetch updated list of subcategories
    const subCategories = await SubCategory.find().sort({ updatedAt: -1 });

    return {
      success: true,
      message:
        "Successfully deleted Sub Category and its associated images in Cloudinary.",
      subCategories: JSON.parse(JSON.stringify(subCategories)),
    };
  } catch (error: any) {
...
// get sub categories by category parent for admin
export const getSubCategoriesByCategoryParent = async (category: string) => {
  try {
    ...
    // Fetch only 'name' and 'id' fields from the SubCategory collection
